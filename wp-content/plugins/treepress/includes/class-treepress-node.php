...
class Treepress_Node {
	...
	public function get_html($the_family) {
		...
		$html .= '
		<tr>
			<td width="33.333333%" style="vertical-align:bottom">
				<b>
					<a href="'.$this->url.'">';
		...
		if(class_exists('TreepressGallery')){
			...
			$Obje = get_post_meta($this->post_id, 'Obje');
			...
			if (is_array($attachments) && !empty($attachments)) {
				...
				foreach ($attachments as $attachment) {
					if( ! in_array($attachment->ID, $skarr ) ){
						if( in_array($attachment->post_mime_type, array('image/jpeg','image/gif','image/jpg','image/png') ) ) {
							$html .= '
							<li>
								<a href="">
									<img style="max-height: 150px;max-width: 100%; width: auto;" src="'.wp_get_attachment_url( $attachment->ID ).'">
								</a>
								<p class="treepress_gallery_gallery_caption">'.get_post_meta($attachment->ID, '_wp_attachment_image_alt', true).'</p>
							</li>';
						} else {
							$html .= '
							<li>
								<a href="'.wp_get_attachment_url( $attachment->ID ).'">
	...
	public function get_toolbar_div() {
		$treepress = new Treepress;
		$plugloc = treepress_plugin_dir_url;
		$terms = wp_get_post_terms($this->post_id, 'family');
		if($terms){
	...
	public function get_thumbnail_div() {
		$plugloc = treepress_plugin_dir_url;
		$html = '';
	...
	public function get_box_html($the_family) {
		$html = '';
