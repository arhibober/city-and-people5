...
class Treepress_Admin
{
        ...
        if ( isset( $_POST['submit'] ) ) {
            ...
            /*
            			foreach($gedcom->getObje() as $obje) {
            				...
            				$file = $this->mb_basename($obje->getFile());
            
            				if(isset($file) && $file) {
            
            					$file_path = $wp_upload_dir['basedir'] . '/treepress/' . $file;
            					$file_url = $wp_upload_dir['baseurl'] . '/treepress/' . $file;
            
            					$file_path_new = $wp_upload_dir['path'] . '/' . $file;
            					$file_url_new = $wp_upload_dir['url'] . '/' . $file;
            
            					if(file_exists($file_path)) {
            
            						copy($file_path, $file_path_new);
            
            						$file_name = basename( $file_url_new );
            						$file_type = wp_check_filetype( $file_name, null );
            						$attachment_title = sanitize_file_name( $obje->getTitl() );
            						...
            						$attach_id = wp_insert_attachment( $post_info, $file_path_new );
            
            						update_post_meta( $attach_id, '_wp_attachment_image_alt', wp_slash( $obje->getTitl() ));
            
            						$mapattach[$obje->getId()] = $attach_id;
            
            						//update_post_meta( $attach_id, '_wp_attachment_obje_id', wp_slash( $obje->getId() ));
            					}
            				}
            			}
            */
            foreach ( $gedcom->getIndi() as $individual ) {
                ...
                foreach ( $Obje as $key => $value ) {
                    $individuals['persons'][$individual->getId()]['Obje'][] = $value->getObje();
                }
            ...
            foreach ( $gedcom->getFam() as $fam ) {
                ...
                $Obje = $fam->getObje();
                foreach ( $Obje as $key => $value ) {
                    $individuals['families'][$fam->getId()]['Obje'][] = $value->getObje();
                }
                ...
                foreach ( $persons as $key => $person ) {
                    ...
                    if ( $person['Obje'] ) {
                        foreach ( $person['Obje'] as $key => $Obje ) {
                            //$attach_id = $mapattach[$Obje];
                            //add_post_meta($post_id, 'Obje', $attach_id);
                        }
                    }
                    if ( $person['event'] ) {
    ...
    public function options_panel()
    {
        ...
        if ( isset( $_POST['update_options'] ) ) {
            ...
            $TreeType = ( isset( $_POST['TreeType'] ) ? sanitize_text_field( $_POST['TreeType'] ) : 'svg' );
            if ( function_exists( 'check_admin_referer' ) ) {
                check_admin_referer( 'family-tree-action_options' );
            }
            
            if ( $active_tab == 'to' ) {
                update_option( 'TreeType', $TreeType );
        ...
        if ( $active_tab == 'dyi' ) {
            ...
            ?></p>
			<p><?php 
            echo  __( 'Shortcode format 1 = ', 'treepress' ) . ' <code>[family-tree family=\'<span style="color:red;">{family_id}</span>\']</code> ' ;
            ?></p>
			<p><?php 
            ...
            ?></p>
			<p><?php 
            echo  __( 'Shortcode format 2 = ', 'treepress' ) . '<code>[family-tree family=\'<span style="color:red;">{family_id}</span>\' root=\'<span style="color:red;">{oldest_member_id_of_family}</span>\'] </code>' ;
            ?></p>
			<p><?php 
            ...
            ?></h3>
			<p><?php 
            echo  __( 'To create a list of all family members, use the shortcode format', 'treepress' ) . ' <code>[family-members family=\'<span style="color:red;">{family_id}</span>\']</code>' ;
            ?></p>
			<p><?php 
            ...
            ?></h3>
			<p><?php 
            _e( 'You can find the family_ID on the Families page - when added, the code \'<span style="color:red;">{family_id}</span>\' will look something like \'42\'.  ', 'treepress' );
            ?></p>
			<p><?php 
            ...
            ?> /></td>
					...
					<td valign="top">
						<table class="form-table">
							...
							<tr valign="top">
								<th scope="row"><label for="bShowGender"><?php 
            _e( 'Tree Style', 'treepress' );
            ?></label></th>
								<td>
									<select name="TreeType">
										<option <?php 
            echo  ( $treepress->options->get_option( 'TreeType' ) == 'html' ? ' selected' : '' ) ;
            ?>  value="html">Html</option>
										<option <?php 
            echo  ( $treepress->options->get_option( 'TreeType' ) == 'svg' ? ' selected' : '' ) ;
...
<!-- 							<tr valign="top">
    ...
    public function render_meta_box_member_events( $post )
    {
        ...
        if ( !get_post_meta( $post->ID, 'ifarray_item', true ) ) {
            $dataf = '';
            $dataf .= $post->post_content;
            foreach ( $ifarray as $key => $ifarray_item ) {
                if ( get_post_meta( $post->ID, strtolower( str_replace( ' ', '_', $ifarray_item['name'] ) ), true ) ) {
                    $dataf .= $ifarray_item['name'] . ': ' . get_post_meta( $post->ID, strtolower( str_replace( ' ', '_', $ifarray_item['name'] ) ), true );
                }
            }
            $my_post = array(
                'ID'           => $post->ID,
            ...
            add_post_meta( $post->ID, 'ifarray_item', true );
            foreach ( $ifarray as $key => $ifarray_item ) {
                if ( get_post_meta( $post->ID, strtolower( str_replace( ' ', '_', $ifarray_item['name'] ) ), true ) ) {
                    delete_post_meta( $post->ID, strtolower( str_replace( ' ', '_', $ifarray_item['name'] ) ) );
                }
            }
        }
    ...
    public function treepress_gallery_show_custom_meta_box( $object )
    {
        ...
        if ( class_exists( 'TreepressGallery' ) ) {
            global  $post ;
            $Obje = get_post_meta( $post->ID, 'Obje' );
            $args = array(
            ...
            if ( is_array( $attachments ) ) {
                $html = '<ul class="treepress-gallery-admin">';
                foreach ( $attachments as $attachment ) {
                    
                    if ( !in_array( $attachment->ID, $skarr ) ) {
                        ...
                        if ( in_array( $attachment->post_mime_type, array(
                            ...
                            $html .= '
						<li><span class="upload-tp-media-rmv">&times;</span> 
							<input name="Obje[]" value="' . $attachment->ID . '" type="hidden" style="width: 100%;margin-bottom: 5px;">
							<img width="150" style="max-height: 150px;max-width: 150px; width: auto;" src="' . wp_get_attachment_url( $attachment->ID ) . '">
						</li>';
                        } else {
                            $html .= '<li><span class="upload-tp-media-rmv">&times;</span> 
							<input name="Obje[]" value="' . $attachment->ID . '" type="hidden" style="width: 100%;margin-bottom: 5px;">
							' . get_post_meta( $attachment->ID, '_wp_attachment_image_alt', true ) . '</li>';
                        }
                        
                        array_push( $skarr, $attachment->ID );
                    }
                
                }
                $html .= '
				<li class="upload-tp-media">
					<svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 150 150">
						<rect fill="#ddd" width="150" height="150"/>
						<text fill="rgba(0,0,0,0.5)" font-family="sans-serif" font-size="16" dy="10.5" x="50%" y="50%" text-anchor="middle">Upload New</text>
					</svg>
				</li>';
                $html .= '</ul>';
            }
    ...
    public function render_meta_box_member()
    {
        ...
        $spouse_divorced = get_post_meta( $post->ID, 'spouse_divorced', true );
        ...
        ?>
				...
				<td>
					<input type="text" name="born" value="<?php 
        ...
        ?>
				...
				<td>
					<input type="text" name="died" value="<?php 
        ...
        ?>
					...
					<span>
						<label>
							<input type="checkbox" name="spouse_divorced" <?php 
        if ( $spouse_divorced && $spouse_divorced == 'on' ) {
            echo  "checked" ;
        }
        ?>> Divorced
						</label>
    ...
    public function treepress_gallery_save_custom_meta( $post_id )
    {
        
        if ( class_exists( 'TreepressGallery' ) ) {
            global  $custom_meta_fields ;
            foreach ( $custom_meta_fields as $field ) {
                $new_meta_value = ( isset( $_POST['treepress_gallery_gallery'] ) ? esc_attr( $_POST['treepress_gallery_gallery'] ) : '' );
                $new_meta_caption_value = ( isset( $_POST['treepress_gallery_gallery_title'] ) ? esc_attr( $_POST['treepress_gallery_gallery_title'] ) : '' );
    ...
    public function save_post_member( $id )
    {
        ...
        if ( isset( $_POST['spouse_divorced'] ) ) {
            $spouse_divorced = stripslashes( strip_tags( $_POST['spouse_divorced'] ) );
        }
        ...
        if ( !empty($father) ) {
            ...
            delete_post_meta( $id, 'father' );
        }
        
        
        if ( !empty($spouse) ) {
            update_post_meta( $id, 'spouse', $spouse );
            update_post_meta( $spouse, 'spouse', $id );
        } else {
            $sp = get_post_meta( $id, 'spouse', true );
            delete_post_meta( $sp, 'spouse' );
            delete_post_meta( $id, 'spouse' );
        }
        
        
        if ( !empty($mother) && !empty($father) ) {
            update_post_meta( $mother, 'spouse', $father );
            update_post_meta( $father, 'spouse', $mother );
        }
        
        
        if ( !empty($spouse_divorced) ) {
            update_post_meta( $id, 'spouse_divorced', $spouse_divorced );
        } else {
            delete_post_meta( $id, 'spouse_divorced' );
        }
    ...
    public function member_posts_born_column( $column, $post_id )
    {
        switch ( $column ) {
            ...
            case 'mother':
                ...
                if ( $mid ) {
                    echo  '<a href="' . get_edit_post_link( $mid ) . '">' . get_the_title( $mid ) . '</a>' ;
                }
                break;
            case 'born':
    ...
    public function treepress_connect()
    {
        register_rest_route( 'treepress/v1', '/connect', array(
            ...
            'permission_callback' => array( $this, 'treepress_permission_callback' ),
        ) );
    }
    ...
    public function get_family_by_family( $family )
    {
        ...
        if ( $posts ) {
            foreach ( $posts as $key => $post ) {
                ...
                $post->spouse_divorced = get_post_meta( $post->ID, 'spouse_divorced', true );
    ...
    public function treepress_permission_callback( $data )
    {
        return true;
    }
